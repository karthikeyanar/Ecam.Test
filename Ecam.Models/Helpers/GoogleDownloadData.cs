...
namespace Ecam.Models
{
    public class GoogleDownloadData
    {
        ...
        public void GoogleDataDownload(string symbol)
        {
            ...
            if (string.IsNullOrEmpty(html) == false)
            {
                try
                {
                    string startWord = "<table border=\"0\" width=\"1%\" style=\"display:inline-table;margin-top:-5px;padding-bottom:0\">";
                    string endWord = "<div class=\"A8ul6\" style=\"padding-bottom:3px\">";
                    ...
                    {
                        startWord = "<div id=market-data-div class=\"id-market-data-div nwp g-floatfix\">";
                        endWord = "<div id=\"sharebox-data\"";
   ...
   | RegexOptions.Compiled
                            ...
                            if (tdCollections.Count > 0)
                            {
                                ...
                                try {
                                    firstCell = TradeHelper.RemoveHTMLTag(tdCollections[0].Groups[2].Value).Trim();
                                } catch { }
                                string secondCell = string.Empty;
                                try { 
                                    secondCell = TradeHelper.RemoveHTMLTag(tdCollections[1].Groups[2].Value).Trim();
                                }catch { }
                                if(firstCell.Contains("Disclaimer")) {
                                   tradeDate = GetTradeDate(firstCell);
                                   GetPrice(tdCollections[0].Groups[2].Value,ref currentPrice,ref change,ref prevPrice);
                                }
                                string[] arr;
                                switch (firstCell)
                                {
                                    ...
                                    case "Open:":
                                        ...
                                        break;
                                    case "High:":
                                        highPrice = DataTypeHelper.ToDecimal(secondCell);
                                        break;
                                    case "Low:":
                                        lowPrice = DataTypeHelper.ToDecimal(secondCell);
                                        break;
                                    case "Mkt cap":
                                    case "Mkt Cap:":
        ...
        private DateTime GetTradeDate(string html) {
            DateTime tradeDate = Convert.ToDateTime("01/01/1900");
            Regex regex = new Regex(
    @"\d{1,2}\s+\w{3},\s+\d{1,2}:\d{1,2}\s+\w{2}",
    RegexOptions.IgnoreCase
    | RegexOptions.Multiline
    | RegexOptions.IgnorePatternWhitespace
    | RegexOptions.Compiled
            ...
            if(collection.Count>0) {
                string s = collection[0].Value;
                if(string.IsNullOrEmpty(s)==false) {
                    string[] arr = s.Split((",").ToCharArray());
                    if(arr.Length>=1) {
                        string[] cells = arr[0].Split((" ").ToCharArray());
                        if(cells.Length>=1) {
                            string datevalue  = cells[0] + "/" + cells[1] + "/" + DateTime.Now.Date.Year;
                            tradeDate = DataTypeHelper.ToDateTime(datevalue);
                        }
                    }
                }
            }
         ...
         private void GetPrice(string html,ref decimal currentPrice,ref decimal change,ref decimal prevPrice) {
         Regex regex = new Regex(
    @"<b(.*?)>(.*?)</b>",
    RegexOptions.IgnoreCase
    | RegexOptions.Multiline
    ...
    | RegexOptions.Compiled
            ...
            if(collection.Count>=2) {
                 currentPrice = DataTypeHelper.ToDecimal(TradeHelper.RemoveHTMLTag(collection[0].Value));
                change = DataTypeHelper.ToDecimal(TradeHelper.RemoveHTMLTag(collection[1].Value));
                prevPrice = currentPrice - change;
            } 
        }

        public void _GoogleDataDownload(string symbol)
        {
            if (string.IsNullOrEmpty(symbol) == true) { return; }
            Random rnd = new Random();
            string url = string.Empty;
