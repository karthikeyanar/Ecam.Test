...
namespace Ecam.Views {
    ...
    public abstract class TemplateVirtualPathProviderViewEngine:IViewEngine {
        // format is ":ViewCacheEntry:{cacheType}:{template}:{prefix}:{name}:{controllerName}:{areaName}:"
        ...
        private string CreateCacheKey(string template,string prefix,string name,string controllerName,string areaName) {
            return string.Format(CultureInfo.InvariantCulture,_cacheKeyFormat,GetType().AssemblyQualifiedName,template,prefix,name,controllerName,areaName);
        }
        ...
        private string GetPath(ControllerContext controllerContext,IEnumerable<string> locations,IEnumerable<string> areaLocations,string locationsPropertyName,string name,string controllerName,string cacheKeyPrefix,bool useCache,bool checkPathValidity,ref bool incompleteMatch,out string[] searchedLocations) {
            ...
            string template = CurrentTemplate(controllerContext.HttpContext);
            ...
            string cacheKey = CreateCacheKey(template,cacheKeyPrefix,name,(nameRepresentsPath) ? string.Empty : controllerName,areaName);
            ...
            return nameRepresentsPath ?
                   ...
                   GetPathFromGeneralName(controllerContext,viewLocations,name,controllerName,areaName,template,cacheKey,ref searchedLocations);
        }

        private string GetPathFromGeneralName(ControllerContext controllerContext,IList<ViewLocation> locations,string name,string controllerName,string areaName,string template,string cacheKey,ref string[] searchedLocations) {
            string result = string.Empty;
            searchedLocations = new string[locations.Count];

            for(int i = 0;i < locations.Count;i++) {
                ViewLocation location = locations[i];
                string virtualPath = location.Format(name,controllerName,areaName,template);
        ...
        private class AreaAwareViewLocation:ViewLocation {
            ...
            public override string Format(string viewName,string controllerName,string areaName,string template) {
                return string.Format(CultureInfo.InvariantCulture,_virtualPathFormatString,viewName,controllerName,areaName,template);
            }
        }
        ...
        private class ViewLocation {
            ...
            public virtual string Format(string viewName,string controllerName,string areaName,string template) {
                return string.Format(CultureInfo.InvariantCulture,_virtualPathFormatString,viewName,controllerName,template);
            }
        }
    }
