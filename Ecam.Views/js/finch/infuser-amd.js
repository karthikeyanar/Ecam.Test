define(['jquery','../finch/TrafficCop-amd.min'],function($,trafficcop) { // You might need to set up a config for require.js for the jquery/trafficcop dependencies to work, or replace it with an absolute path
    /*
        infuser.js
        Author: Jim Cowart
    ...
    */
    var hashStorage={
        templates: {},
        convertTemplateId: function(templateId) {
            return SH(templateId).replaceAll('/','_').replaceAll('?','_').replaceAll('=','_').replaceAll('.','_').replaceAll('#','_').s;
        },
        storeTemplate: function(templateId,templateContent) {
            var keys=$.localStorage.keys();
            templateId=this.convertTemplateId(templateId);
            var templateKey=cString(templateId.split("_v_")[0]);
            if(templateKey!="") {
                $.each(keys,function(i,key) {
                    if(key!=templateId) {
                        var arr=key.split("_v_");
                        if(arr[0]==templateKey) {
            ...
            try {
                setLS(templateId,templateContent);
            } catch(ex) {
                console.log('storeTemplate ex=',ex);
            }
        ...
        getTemplate: function(templateId) {
            templateId=this.convertTemplateId(templateId);
            try {
                var cnt=cString(getLS(templateId));
            } catch(ex) {
                console.log('getTemplate ex=',ex);
            }
            return cnt;
    ...
    var infuser={
        ...
        get: function(options,callback) {
            var templateOptions=$.extend({},infuser.defaults,(typeof options==="object"?options:{ templateId: options })),
                template;
            templateOptions.ajax.url=helpers.getTemplatePath(templateOptions)+"?v="+_Version;
