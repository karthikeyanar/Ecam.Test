...
namespace Ecam.AlertEmail
{
    class Program
    {
        ...
        {
            List<TempNiftyListClass> list = null;
            list = GetCompanies("ind_nifty50list.csv");
            UpdateNiftySlaps(list, 50);
            list = GetCompanies("ind_nifty100list.csv");
            UpdateNiftySlaps(list, 100);
            list = GetCompanies("ind_nifty200list.csv");
            UpdateNiftySlaps(list, 200);
            Console.WriteLine("Press any key to exit");
            Console.ReadLine();
        }

        private static List<TempNiftyListClass> GetCompanies(string import_company_csv_file_name)
        {
            List<TempNiftyListClass> list = new List<TempNiftyListClass>();
            ...
            using (TextReader reader = File.OpenText(sFile))
            {
                ...
                while (csv.Read())
                {
                    ...
                    list.Add(new TempNiftyListClass { company_name = companyName, industry = industry, symbol = symbol });
                }
            }
            return list;
        }

        private static void UpdateNiftySlaps(List<TempNiftyListClass> list, int slap)
        {
            int i = 0;
            string sql = "";
            ...
            {
                case 50:
                    sql = string.Format("update tra_company set is_nifty_50=0;");
                    break;
                case 100:
                    sql = string.Format("update tra_company set is_nifty_100=0;");
                    break;
                case 200:
                    sql = string.Format("update tra_company set is_nifty_200=0;");
                    break;
            }
        ...
        private static void ImportCompanies(List<TempNiftyListClass> list)
        {
            int i = 0;
            using (EcamContext context = new EcamContext())
            {
                foreach (var row in list)
                {
                    ...
                    tra_category category = (from q in context.tra_category
                                             where q.category_name == row.industry
                                             select q).FirstOrDefault();
                    if (category == null)
                    {
                        category = new tra_category { category_name = row.industry };
    ...
    public class TempNiftyListClass
    {
